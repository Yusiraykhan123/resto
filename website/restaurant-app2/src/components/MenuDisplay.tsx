...
export const MenuDisplay: React.FC = () => {
  ...
  const getImageUrl = (imagePath: string | null) => {
    if (!imagePath) {
      return '/placeholder-image.jpg'; // Path gambar placeholder jika tidak ada gambar
    }
    return imagePath.startsWith('http')
      ? imagePath
      : `https://yoloverse.pythonanywhere.com${imagePath}`; // Menambahkan base URL jika perlu
  };

  if (isLoading) return <div className="text-center py-10">Loading menu items...</div>;
                  ...
                  .map((item) => (
                    <div key={item.id} className="mb-6 last:mb-0">
                      <div className="flex items-center gap-4">
                        <img 
                          src={getImageUrl(item.image)} // Validasi URL gambar
                          alt={item.name} 
